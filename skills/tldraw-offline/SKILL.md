...
<!-- WSL ADAPTATION (installed for Claude Code in WSL2, NAT mode): the tldraw app runs on
Windows and binds 127.0.0.1, which WSL cannot reach directly. EVERY http request in this skill
must go through Windows curl: /mnt/c/Windows/System32/curl.exe (drop-in replacement for `curl`
in all examples below), or simply use the tq helper at $HOME/.claude/skills/tldraw-offline/tq,
which already does this. server.json is read via the /mnt/c path. -->
...
The default server is `http://localhost:7236`. If that port is not active, read the `port` from `/mnt/c/Users/IvanKoh/AppData/Roaming/tldraw/server.json`.
...
```bash
PORT=$(jq -r .port '/mnt/c/Users/IvanKoh/AppData/Roaming/tldraw/server.json'); TOKEN=$(jq -r .token '/mnt/c/Users/IvanKoh/AppData/Roaming/tldraw/server.json')
# use as:  http://localhost:$PORT/...   -H "authorization: Bearer $TOKEN"
```

### Helper: `tq`

A ready-made helper ships with this skill at `"$HOME/.claude/skills/tldraw-offline/tq"`. Invoke it as `sh "$HOME/.claude/skills/tldraw-offline/tq" <METHOD> <path> [body]` — it re-reads the port and token from `server.json` itself on every call, so you never handle the token or the fresh-shell env problem. A body starting with `{` is sent as JSON; anything else as raw `text/plain`:

```bash
sh "$HOME/.claude/skills/tldraw-offline/tq" POST /api/search '{"code":"return await api.getDocs()"}'
sh "$HOME/.claude/skills/tldraw-offline/tq" POST /api/doc/DOC_ID/exec 'return editor.getCurrentPageShapes().length'
sh "$HOME/.claude/skills/tldraw-offline/tq" GET  /api/doc/DOC_ID/script-status
```

If `tq` is missing (an older install), fall back to raw `curl` with the `PORT`/`TOKEN` reads shown above. The raw-`curl` examples below stay in explicit form so each request is visible; translate any to `sh "$HOME/.claude/skills/tldraw-offline/tq" <METHOD> <path> [body]`.
...
Shown as raw `curl`; `sh "$HOME/.claude/skills/tldraw-offline/tq" <METHOD> <path> [body]` is the shorter equivalent that handles the port and token for you.
